...
A tiny, self-contained example task for AutoAS, modelled on
...
| File | Role | Editable? |
| --- | --- | --- |
| `solution.py` | The solver AutoAS optimises (`solve(problem)`). | **Yes — this is the only edit surface.** |
...
AutoAS iterates on is never the data it is finally judged on:
...
AutoAS runs experiments in **git worktrees off the repo root**, so run the example
from a **copy outside the AutoAS checkout** — that keeps your AutoAS clone's git
history clean and avoids preflight failures from the surrounding repo:

```bash
cp -r examples/algotune_knn /tmp/algotune_knn   # copy out of the AutoAS repo
cd /tmp/algotune_knn
git init -q && git add -A && git commit -qm "baseline"  # AutoAS wants a clean repo
autoas   # then in the intake chat, confirm the contract below
```
