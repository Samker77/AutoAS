...
async def run_intake(
    *,
    ...
    ``<starting_cwd>/.autoas/conversations/`` so it can be continued later. With
    ``continue_latest`` (``autoas --continue``) the newest unfinished
...
def _print_llm_failure_hint(*, provider: str, model: str | None, base_url: str | None) -> None:
    ...
    lines = [
        "The planning model did not answer, so AutoAS stayed in intake instead of launching a run.",
        "",
    ...
    lines.extend([
        "",
        "Check that the endpoint is running and serving that model, or update `autoas config init ... --force`.",
...
def _print_resumed_history(messages: list[dict]) -> None:
    ...
    for m in messages:
        ...
        if role == "user":
            ...
            _console.print(f"[cyan]autoas ›[/cyan] {escape(text)}")
    if rendered:
    ...
    *,
    ...
    if cmd == "/help":
        ...
        if not convs and not runs:
            ...
            _console.print(
                "  [dim]Conversations live in [/dim][cyan].autoas/conversations/[/cyan]"
                "[dim] and launched runs in [/dim][cyan].autoas/sessions/[/cyan][dim], "
