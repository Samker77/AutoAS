    ...
    input_schema = {
        ...
        "properties": {
            ...
            "suggested_max_cycles": {
                "type": "integer",
                "description": "How many autoas cycles to budget. Conservative for small benchmarks (1-3).",
            },
            "suggested_max_turns": {
                "type": "integer",
...
def _with_experience(cwd: str, instruction: str, apply: Any, sessions: Any = None) -> str:
    ...
    if not apply:
        return instruction
    try:
        from ...recall import compose_for_topic_semantic, compose_from_sessions
        block = ""
        if isinstance(sessions, list) and sessions:
            block = compose_from_sessions(cwd, [str(s) for s in sessions])
        if not block:
            # E2: bullet-level semantic recall (MemoryIndex) instead of the old
            # whole-file keyword gate. Empty when nothing matches — caller keeps
            # the raw instruction, same contract as the old compose_for_topic.
            block = compose_for_topic_semantic(cwd, instruction)
