...
The intake REPL (``autoas`` with no subcommand) is a planning chat held *before*
...
runs persist under ``.autoas/sessions/<run_name>/`` (see
:mod:`autoas.coordinator.checkpoint`); this module is the analogous, deliberately
...
(:func:`autoas.coordinator.checkpoint.write_messages` /
:func:`~autoas.coordinator.checkpoint.read_messages`) so a conversation and a run
...
def conversations_root(cwd: str | os.PathLike[str]) -> Path:
    """Return ``<cwd>/.autoas/conversations`` (the per-project conversation root)."""
...
def _messages_for_disk(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    user authorized AutoAS to inspect.
    """
