"""``autoas idea-check`` — zero-config novelty / prior-art check for one idea.
...
def idea_check_command(
    ...
    # (~/.autoas/config.yaml: provider / model / base_url / api_key / openai_api),
    # the same source `autoas run` uses, then force the alphaXiv backend on.
    ...
    try:
        ...
        typer.secho(
            ...
            f"(run `autoas login` or `autoas setup`): {exc}",
            fg=typer.colors.RED,
            err=True,
        )
        raise typer.Exit(code=1) from exc
