"""``autoas mcp`` — run AutoAS's deterministic, keyless tools as an MCP server.

This lets any MCP-capable coding agent (Claude Code, Codex, …) drive a full
AutoAS research workflow *using its own model* — no AutoAS API key, no separate
runtime. Register it with, e.g.::

    claude mcp add autoas -- autoas mcp
...
def mcp_command() -> None:
    """Start the AutoAS MCP server (stdio transport)."""
    # Imported lazily so `autoas --help` and unrelated commands never pay for the
