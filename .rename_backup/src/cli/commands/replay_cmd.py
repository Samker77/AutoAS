"""`autoas replay` — replay a recorded session through the live dashboard.

Zero-setup way to *see AutoAS work*: the live agent needs an LLM key, but
replaying a recorded run needs nothing. ``autoas replay --demo`` plays a bundled
sample; ``autoas replay <session>`` plays any real run's ``events.jsonl``.
"""
...
def replay_command(
    ...
    out: Optional[Path] = typer.Option(
        None, "--out", "-o", help="Output path for --html (default: <session>/autoas-tree.html).",
    ),
    ...
    """Replay a previous AutoAS session — watch the idea tree grow with zero setup."""
...
def _resolve_source(source: Path, cwd: Path) -> Path:
    ...
    conventional sessions directory so ``autoas replay my-run`` works the same way
    ``autoas export my-run`` does.
    """
