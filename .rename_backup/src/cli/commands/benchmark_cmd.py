"""`autoas benchmark` — verify, list, scaffold, and collect Task Packs in the zoo.

Subcommands:

* ``autoas benchmark verify <pack-dir>`` — structurally check a benchmark folder
  (README + PROVENANCE + eval entrypoint present). Does not run the eval.
* ``autoas benchmark list [zoo-dir]`` — print a plain index of packs (not a ranked
  leaderboard).
* ``autoas benchmark scaffold <dir>`` — write the measurement plumbing (light) or a
  full zoo benchmark (zoo) into an existing local directory.
* ``autoas benchmark add "<request>" | <url>`` — turn a one-line request into a runnable
...
benchmark_app = typer.Typer(
    name="benchmark",
    help="Verify and list Task Packs in the autoas-zoo benchmark format.",
...
def _user_runner(*, with_search: bool, ask_user: bool = False):
    """Build an agent runner that uses the user's configured provider (~/.autoas/config.yaml),
    so the collection agents inherit the same LLM as `autoas run` (e.g. openai-oauth/gpt-5.5).
...
def list_command(
    zoo_dir: Path = typer.Argument(
        Path("autoas-zoo"),
        help="The zoo directory to index (default: ./autoas-zoo).",
    ),
) -> None:
...
def scaffold_command(
    ...
    """Scaffold the AutoAS-ready reference folder (light) or a full zoo benchmark (zoo)."""
    ...
    if git_init and not (target / ".git").exists():
        ...
        subprocess.run(
            ["git", "-c", "user.email=autoas@localhost", "-c", "user.name=AutoAS",
             "commit", "-m", "baseline: scaffold AutoAS benchmark structure"],
...
def add_command(
    ...
    name: str | None = typer.Option(
        None, "--name", "-n",
        help="Pack name (the autoas-zoo/<name> folder). Optional for a query (discovery suggests one).",
    ),
    dest: Path = typer.Option(
        Path("autoas-zoo"), "--dest",
        help="Where to write the draft pack (default: ./autoas-zoo).",
    ),
    ...
    if from_query:
        ...
        try:
            disc = asyncio.run(discover(
                ...
                work_dir=Path(tempfile.mkdtemp(prefix="autoas-discover-")),
                max_turns=max_turns,
            ))
        except Exception as exc:  # noqa: BLE001
            typer.secho(f"  discovery could not start: {exc}", fg=typer.colors.RED, err=True)
            typer.echo("  (configure a provider with `autoas setup` / set your API key)")
    ...
    if (from_query or do_bringup) and result.draft_pack_dir:
        ...
        try:
            ...
            typer.echo("  (configure a provider with `autoas setup` / set your API key, then retry)")
            raise typer.Exit(code=1) from exc
