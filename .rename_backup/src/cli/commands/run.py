"""`autoas run` — chat with the intake agent, then run the coordinator."""
...
_AUTO_CONFIG_NAMES = ("research_config.yaml", "autoas.yaml", "autoresearch.yaml")
...
def run_command(
    ...
    config: Path | None = typer.Option(
        None, "--config", "-c",
        help="Project YAML config. Defaults to research_config.yaml / autoas.yaml / autoresearch.yaml in the target project.",
    ),
    ...
    run_name: str | None = typer.Option(None, "--run-name", help="Session name under .autoas/sessions/. Defaults to timestamp."),
    ...
    # user through `autoas setup` once instead of failing on missing credentials,
    ...
    # from autoas setup / project YAML; run-time CLI only overrides execution
    ...
    # compose inline, and nothing flickers. AUTOAS_DASHBOARD_INPUT_MODE=raw|line
    ...
    requested_dashboard_input_mode = os.environ.get("AUTOAS_DASHBOARD_INPUT_MODE", "").strip().lower()
    ...
    # The previous "autoas run" banner panel (task/cwd/model/cycles/...)
...
def _live_input_unsafe_terminal(env: "os._Environ[str] | dict[str, str]") -> bool:
    ...
    ``AUTOAS_FORCE_DASHBOARD_INPUT`` (1/true/yes) forces raw input back on for
    users who know their setup is fine.
    """
    if env.get("AUTOAS_FORCE_DASHBOARD_INPUT", "").strip().lower() in ("1", "true", "yes"):
        return False
...
def _resolve_dashboard_input_mode(env: "os._Environ[str] | dict[str, str]") -> str:
    ...
    """
    requested = env.get("AUTOAS_DASHBOARD_INPUT_MODE", "").strip().lower()
    if requested == "prompt":
        requested = "app"
...
def _add_effective_llm_overrides(overrides: dict[str, Any], eff: dict[str, Any]) -> None:
    """Copy LLM values resolved from autoas setup/project YAML into config overrides.

    ``autoas run`` no longer accepts model/provider flags, but the final
    CoordinatorConfig must still receive the values selected by ``autoas setup``
