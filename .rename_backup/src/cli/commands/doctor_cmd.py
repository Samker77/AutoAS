"""`autoas doctor` — diagnose install + runtime environment."""
...
def doctor_command() -> None:
    ...
    #     `autoas` from a fresh prompt. If these disagree, there's a
    #     shadowing autoas higher on PATH and the user is likely confused.
    ...
    # ── package import ───────────────────────────────────────────
    try:
        import autoas  # noqa: F401
        _ok(f"autoas imports from {Path(autoas.__file__).parent}")
    except Exception as e:
        _fail(f"cannot import autoas: {e!r}")
        problems += 1
