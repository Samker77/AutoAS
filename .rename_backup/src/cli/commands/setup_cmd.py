"""`autoas setup` — interactive one-time configuration wizard.

A first-time user runs ``autoas`` (or ``autoas setup``) and answers a few prompts;
we write ``~/.autoas/config.yaml`` so subsequent runs need no flags. The wizard is
the interactive sibling of the flag-driven ``autoas config init`` and shares its
...
def run_setup_wizard(*, force: bool = False) -> bool:
    ...
    :func:`write_user_llm_config` so the file matches ``autoas config init``.
    """
    ...
    if GLOBAL_CONFIG_FILE.exists() and not force:
        ...
        if not typer.confirm("Overwrite it?", default=False):
            _console.print("[dim]Keeping the existing config. Run `autoas config show` to view it.[/]")
            return False

    _console.print()
    _console.print("[bold cyan]autoas setup[/] — let's configure your model (one time).")
    ...
    if easy_llm is not None:
        ...
        _console.print(
            ...
            "([dim]view it anytime with[/] [bold]autoas config show[/])."
        )
        _console.print("Just run [bold]autoas[/] to start a session.\n")
        return True
    ...
    provider = _select_choice(
        "API type",
        options=[
            ("auto", "let AutoAS detect it — probes Responses API, else chat completions"),
...
def _choose_easy_start() -> dict[str, str] | None:
    ...
    _console.print(
        "[dim]note: free-tier models are great for trying AutoAS on your own task; "
...
def _setup_openai_oauth() -> bool:
    ...
    _console.print(
        f"\n[green]Done.[/] Saved to [bold]{GLOBAL_CONFIG_FILE}[/]. "
        "Just run [bold]autoas[/] to start a session.\n"
    )
    return True
...
def setup_command(
    ...
    """Interactively configure autoas (writes ~/.autoas/config.yaml)."""
...
def quickstart_command(
    ...
    Same writer as ``autoas setup``; this entry point just leads with the
