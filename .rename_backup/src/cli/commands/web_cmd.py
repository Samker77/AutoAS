"""``autoas web`` — open a read-only browser monitor for a session.

Unlike the live dashboard (which only exists during ``autoas run``), this serves
a session *from disk*, so it works for the keyless, host-driven flow: while a
coding agent drives an AutoAS run via the ``autoas mcp`` tools, the user (or the
...
def web_command(
    session: str = typer.Argument(
        ..., help="Session name or path (under .autoas/sessions/), e.g. the run name."
    ),
    cwd: Path | None = typer.Option(
        None, "--cwd", help="Project directory holding .autoas/sessions/ (default: current directory)."
    ),
    ...
    """Serve a read-only web monitor for an AutoAS session and block until Ctrl-C."""
    ...
    typer.secho(f"\nAutoAS monitor (read-only) → {server.url}", fg=typer.colors.CYAN, bold=True)
