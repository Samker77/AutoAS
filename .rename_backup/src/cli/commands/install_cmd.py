"""``autoas install`` / ``autoas uninstall`` — manage the bundled Agent Skill suite.

AutoAS ships an 11-directory *Agent Skill suite* (``skills/autoas-*``) that lets a
host coding agent (Claude Code, Codex, …) run the AutoAS research workflow using
*its own* model — no AutoAS API key, no separate runtime. Historically users had
to copy those directories by hand (see ``skills/README.md``). These commands
automate that: ``autoas install`` discovers the suite bundled with the installed
package and copies it into the right harness skills directory; ``autoas
uninstall`` removes exactly the directories AutoAS owns and nothing else.

Design notes
------------
* **Idempotent.** Re-running ``install`` refreshes each ``autoas-*`` directory in
...
* **Namespaced + safe.** We only ever touch directories whose names start with
  ``autoas-``. A user's own unrelated skills living in the same target directory
  are never read, copied, or removed.
* **No LLM, no key.** This command performs pure filesystem work; it is part of
  AutoAS's keyless coding-agent integration path.
...
SKILLS_DIR_ENV = "AUTOAS_SKILLS_DIR"


def bundled_skills_root() -> Path:
    """Locate the ``skills/`` suite that ships with this AutoAS install.

    Resolution order (first hit wins):

    1. ``$AUTOAS_SKILLS_DIR`` — explicit override (tests / packaging debug).
    2. ``<autoas package>/skills_suite`` — the location the wheel ships the suite
       to (see ``pyproject.toml`` ``package-dir`` mapping).
    3. ``<repo root>/skills`` — an editable / source checkout, where the suite
       lives one level above the ``autoas`` package (``src/``).
    ...
    import autoas  # local import: avoids a hard import cycle at module load time

    pkg_dir = Path(autoas.__file__).resolve().parent
    ...
    raise FileNotFoundError(
        "Could not locate the bundled AutoAS skill suite. Looked under "
        f"${SKILLS_DIR_ENV}, {packaged}, and {dev}."
    )


def discover_skill_dirs(root: Path) -> list[Path]:
    """Return the ``autoas-*`` skill directories directly under *root*, sorted.
...
def install_skills(src_root: Path, dest: Path) -> list[str]:
    """Copy every ``autoas-*`` skill dir from *src_root* into *dest*.

    Existing ``autoas-*`` directories at the destination are removed first so the
...
def uninstall_skills(dest: Path) -> list[str]:
    """Remove only the ``autoas-*`` skill dirs from *dest*; leave everything else.
...
def install_command(
    ...
    """Install the bundled AutoAS skill suite into a coding-agent skills directory."""
    ...
    for name in installed:
        typer.echo(f"  ✓ {name}")
    typer.secho(
        "\nRestart your coding agent, then run `/autoas-research-agent <your task>`.",
...
def uninstall_command(
    ...
    """Remove the AutoAS skill suite from a coding-agent skills directory."""
    dest = resolve_target(claude=claude, codex=codex, project=project, target=target)
