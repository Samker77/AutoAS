"""`autoas login` — subscription OAuth login (experimental).

Supports ChatGPT (OpenAI) and Claude (Anthropic) subscriptions: ``autoas login
openai`` / ``autoas login claude`` run a browser OAuth flow so subscribers can
drive AutoAS with their subscription instead of a pay-per-token API key.
...
def login_openai(
    ...
    """Log in to ChatGPT and store the subscription token under ~/.autoas/oauth/."""
    ...
    _console.print("Run [bold]autoas[/] to start a session.")
...
def login_claude(
    ...
    """Log in to Claude and store the subscription token under ~/.autoas/oauth/."""
...
def login_status() -> None:
    ...
    if not any_signed_in:
        _console.print(
            "[yellow]not signed in[/] — run `autoas login openai` or `autoas login claude`"
        )
        raise typer.Exit(code=1)
