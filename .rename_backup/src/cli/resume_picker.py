...
Each `autoas` run lives in its own session dir at
``<cwd>/.autoas/sessions/<run_name>/`` with a checkpoint at
``.coordinator/checkpoint.json``. When the user launches `autoas` in a project that
...
def find_resumable_sessions(cwd: Path, *, include_subdirs: bool = False) -> list[ResumableSession]:
    """Return resumable sessions under ``<cwd>/.autoas/sessions``, newest first.
    ...
    *target project* dir (which intake may have redirected to), so a user who
    launched ``autoas`` from a parent folder would otherwise see nothing.
    """
...
def _scan_dir(cwd: Path) -> list[ResumableSession]:
    """Collect resumable sessions directly under ``<cwd>/.autoas/sessions`` (unsorted)."""
