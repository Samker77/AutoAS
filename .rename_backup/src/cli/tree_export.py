...
event bus — in two surfaces. ``autoas replay`` paints the tree in your terminal;
``autoas replay --html`` writes a single self-contained file that animates the
...
def default_html_path(rec: Recording) -> Path:
    ...
    if rec.session_dir is not None:
        return Path(rec.session_dir) / "autoas-tree.html"
    return Path.cwd() / f"autoas-tree-{rec.run_name}.html"
