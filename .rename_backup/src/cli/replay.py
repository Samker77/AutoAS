...
needs nothing. ``autoas replay --demo`` is the zero-setup "see it work" path — the
...
def resolve_events_path(source: Path) -> Path:
    ...
    if source.is_dir():
        ...
        raise FileNotFoundError(
            f"no events.jsonl in {source} — is this an autoas session directory?"
        )
