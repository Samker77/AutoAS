...
class RunDashboard:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        if exc_val is not None:
            ...
            try:
                if isinstance(exc_val, KeyboardInterrupt):
                    msg = "\nautoas: interrupted (Ctrl-C)"
                else:
                    msg = f"\nautoas: crashed — {type(exc_val).__name__}: {exc_val}"
                if path:
    ...
    def _start_line_stdin_reader(self) -> None:
        ...
        self._stdin_thread = threading.Thread(target=run, daemon=True,
                                              name="autoas-dashboard-stdin-line")
        self._stdin_thread.start()
        self._input_active = True
        self.state.mark_dirty()
    ...
    def _render_header(self) -> Panel:
        ...
        title = Text()
        title.append("autoas", style="bold yellow")
    ...
    def _render_ideas(self) -> Padding:
        ...
        # (e.g. the literal autoas cycle root, or out-of-order events) get
...
#
# We write the traceback to ``$AUTOAS_CRASH_LOG`` (or
# ``~/.autoas/crashes/<timestamp>.log`` by default) so we can find out
...
def _dump_crash(where: str, exc: BaseException) -> str | None:
    ...
    The user can set ``AUTOAS_CRASH_LOG`` to a fixed path; otherwise we
    rotate via timestamps so multiple crashes don't overwrite each other.
    """
    import traceback

    try:
        env_path = os.environ.get("AUTOAS_CRASH_LOG")
        if env_path:
            path = os.path.expanduser(env_path)
        else:
            base = os.path.expanduser("~/.autoas/crashes")
