...
**AutoAS 是一个自主科研智能体，它把一个长周期目标转化为持续累积的搜索过程。** 给它一个基准
（benchmark）和一个目标；它会提出假设、修改代码、运行真实实验、从结果中学习，并保留那些在
留出（held-out）数据上确实站得住脚的改进。

不同于“一次性尝试、过后即弃”的做法，AutoAS 会生长出一棵**假设树**：每个想法都是一根分支——
...
| 智能体 | 角色 |
| --- | --- |
| **Coordinator（协调者）** | 科研总监。维护想法树，通过 *autoas cycle* 驱动搜索，并派发实验。 |
