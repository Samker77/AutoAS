...
Every interactive AutoAS run exposes two live views of the same study: a **terminal
dashboard** and a **browser monitor (Web UI)**. Both read from the same event stream, so
they always agree.

## The terminal dashboard

When you start a run in a terminal, AutoAS renders a live dashboard showing the current
...
For interactive runs, AutoAS also starts a small web server that mirrors the run to your
...
| --- | --- |
| **Default port** | `8765`. |
| **Auto-roll** | If `8765` is busy, AutoAS walks forward up to 10 ports (`8765`–`8774`) until one binds. |
...
```bash
autoas --webui-port 9000      # serve the monitor on :9000
autoas --no-webui             # no browser monitor at all
```
