...
Thanks for your interest in improving AutoAS. This page covers the basics of working on the
...
The source lives in `src/` and is imported as the `autoas` package:
...
!!! note "Packaging detail"
    The on-disk directory is `src/`, but it is installed and imported as `autoas`
...
3. Verify the CLI still works (`autoas version`, `autoas doctor`) and, if you touched docs,
   that `mkdocs build` is clean.
4. Open a pull request describing the motivation and the change.

## Citation

If you use AutoAS in your research, please cite the paper:

```bibtex
@misc{jin2026autoas,
  title  = {Toward Generalist Autonomous Research via Hypothesis-Tree Refinement},
  author = {AI Scientist Team and Yuyang Hu and Kai Qiu and Qi Dai and Chong Luo and
