...
A plugin retargets AutoAS to a **domain** without changing any code. It is a single YAML
...
!!! question "Do I even need a plugin?"
    **No, not to start.** For a one-off task, just prepare a repo and launch `autoas` (see
...
Put one line in your project's config (`research_config.yaml`, `autoas.yaml`, or
...
AutoAS auto-discovers the config in the project directory; the intake chat then runs with
...
| `description` | ✓ | One-line summary shown in `autoas` plugin listings. |
...
merge. When an executor starts, AutoAS records a SHA-256 manifest of every
...
`eval_contract.contamination` declares what AutoAS needs to warn when a benchmark's
...
AutoAS ships one plugin, `mle_kaggle`, as a complete worked example for Kaggle / MLE-bench
...
1. Create a `plugins/` folder inside your project and add `my_domain.yaml` there:
   `<project>/plugins/my_domain.yaml`. AutoAS discovers project plugins from this folder
   (built-in plugins live alongside AutoAS and are always available too). Start from the
...
my_domain` in the chat — and launch `autoas`. Pair it with a [Skill](skills.md) when you want
