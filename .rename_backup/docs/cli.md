...
AutoAS installs the `autoas` command (plus a few lower-level entry points). This page is the
complete reference.
...
| --- | --- |
| `autoas` | With no subcommand, behaves like `autoas run` — starts an interactive session in the current directory. |
| `autoas run` | Start an AI-powered research session. |
| `autoas setup` | Interactive wizard to write your provider, model, and API key. |
| `autoas config` | Inspect and manage stored configuration. |
| `autoas doctor` | Diagnose your environment (PATH, Python, git, API keys). |
| `autoas report` | Work with a finished run's report. |
| `autoas idea-check` | Novelty / prior-art check for a single idea against the public alphaXiv API. |
| `autoas version` | Print the installed version. |

!!! tip
    Running `autoas` (or `autoas --cwd .`) with no subcommand is equivalent to `autoas run`.

## `autoas run`

```bash
autoas run [INSTRUCTION] [OPTIONS]
```
...
| `--config, -c PATH` | Project YAML config. Defaults to `research_config.yaml` / `autoas.yaml` / `autoresearch.yaml` in the target project. |
...
| `--run-name NAME` | Session name under `.autoas/sessions/`. Defaults to a timestamp. |
| `--resume` | Resume an interrupted run from its checkpoint in the existing workspace/session. |
| `--workspace-dir PATH` | Session/artifact directory override. Default `<target>/.autoas/sessions/<run_name>`. |
...
autoas idea-check "Use entity-relation scratchpads to improve multi-hop QA"
autoas idea-check "tree search over plans for code generation" --json
```

Zero-config: no search endpoint or alphaXiv API key is required — it queries
alphaXiv's public endpoints directly. Reuses your existing AutoAS LLM
credentials (`autoas login` / API key).
...
For advanced/low-level use, AutoAS also installs:
