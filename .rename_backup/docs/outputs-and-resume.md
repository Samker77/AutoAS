...
AutoAS records everything a run produces so you can inspect it, reproduce it, and pick up
where you left off.
...
!!! tip "Reuse a setup"
    ...
    `config_snapshot.yaml` to your project root as `autoas.yaml` — AutoAS auto-loads it next
    time. See [Preparing a Benchmark → Saving and reusing a setup](preparing-a-benchmark.md#saving-and-reusing-a-setup).

## `REPORT.md`

When a run finishes, AutoAS writes a `REPORT.md` — the human-readable write-up of the
study: what was tried, what worked, the evidence behind each conclusion, and the final
result. Use `autoas report` to work with it, and the `/report` slash command during a run
to print artifact paths.

By default AutoAS then opens a **read-only Q&A prompt** so you can interrogate the finished
...
The intake chat — what you do at the `autoas` prompt *before* a run launches — is itself
...
```text
<cwd>/.autoas/conversations/<conv_id>/
```
...
- **`autoas --continue`** (`-C`) — reload the most recent *unfinished* conversation and keep
...
```bash
autoas --resume --run-name my-study
```
