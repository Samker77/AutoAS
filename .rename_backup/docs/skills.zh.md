...
!!! question "我需要写一个吗？"
    **不需要。** AutoAS 随附了会自动加载的、合理的默认技能。只有当你想改进智能体在某个特定步骤的
    推理时，才写你自己的。
...
1. 在你项目里建文件夹 `.autoas/skills/` 并在其中加一个 markdown 文件，例如
   `<project>/.autoas/skills/my_skill.md`。AutoAS 从这个文件夹发现项目技能；同名 `name`
   的项目技能会覆盖随附的同名技能。
