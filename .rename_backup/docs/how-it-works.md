...
AutoAS turns a long-horizon objective into a structured, cumulative search. This page
explains the core ideas: the two agents, the autoas cycle, the Idea Tree, git isolation,
...
The Idea Tree is AutoAS's persistent memory. Every node is a hypothesis with its status,
...
AutoAS sustain a long study without drowning in its own history.
...
By default AutoAS refuses to start from a non-base branch; use `--allow-non-base-branch`
...
AutoAS runs fully autonomously by default, but you can dial in oversight with interaction
modes:
