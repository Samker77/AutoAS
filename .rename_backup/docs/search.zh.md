...
真实研究从已有的知识出发。AutoAS 能引入外部知识——文献与开放网络——分**两条互相独立的 lane**：
...
```bash
autoas idea-check "一句话描述你的假设"
```

选项见 [`autoas idea-check`](cli.zh.md#autoas-idea-check)。
...
!!! note "Exa via MCP"
    ...
    它需要 MCP 客户端：`pip install 'autoas-agent[mcp]'`。而普通的 `exa` 后端走同一家的 REST API
    （那个**需要** key）——按你的环境二选一。
...
旧配置仍原样可用。当 `backends` 为空时，AutoAS 自动映射旧字段：`builtin_backend: alphaxiv` → `[alphaxiv]`，
...
- [`autoas idea-check`](cli.zh.md#autoas-idea-check) —— 一次性新颖性核查。
