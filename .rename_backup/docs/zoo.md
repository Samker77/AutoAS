...
This page is the exact format for a benchmark folder and what `autoas benchmark verify`
...
is plain prose that AutoAS reads at intake — there is no YAML manifest to fill in.

## What a benchmark folder contains

Each `autoas-zoo/<name>/` holds four things:

| File / dir | Role | For |
| --- | --- | --- |
| `README.md` | What the task is, the metric, what AutoAS may edit, how dev/test differ — in plain language. | AutoAS (and humans) |
| baseline code | The editable baseline — `solution.py`, or a whole set of files. | AutoAS edits |
...
The README is what AutoAS reads to understand the task, the same way its intake reads any
...
2. **The metric** — what the eval prints (one `score:` line) and whether higher or lower is
   better.
3. **What AutoAS may edit** — the baseline file(s); everything else (the eval script,
...
## What `autoas benchmark verify` checks
...
```bash
autoas benchmark verify autoas-zoo/<name>   # exits non-zero if a piece is missing
autoas benchmark list autoas-zoo            # index the benchmarks
```
...
## Run AutoAS on a benchmark

AutoAS runs experiments in git worktrees off the repo root, so work from a copy **outside** the
AutoAS checkout:

```bash
cp -r autoas-zoo/algotune_knn /tmp/algotune_knn
cd /tmp/algotune_knn
git init -q && git add -A && git commit -qm baseline
autoas   # AutoAS reads the README, confirms the task, then iterates
```

## Add a benchmark

1. Scaffold the structure: `autoas benchmark scaffold autoas-zoo/<name> --style zoo`. This writes
   an eval stub, a `solution.py` placeholder, a natural-language `README.md`, and a
   `PROVENANCE.md` — never the solution itself.
2. Fill in the baseline (`solution.py`), the eval (`eval.py`/`eval.sh`), the README (for AutoAS),
   and PROVENANCE (for humans).
3. Run `autoas benchmark verify autoas-zoo/<name>` until it exits 0, then a maintainer accepts it.
   Drafting may be automated; **acceptance is a human step**.

For an end-to-end example, see [`autoas-zoo/algotune_knn`](https://github.com/RUC-NLPIR/AutoAS/tree/main/autoas-zoo/algotune_knn).
