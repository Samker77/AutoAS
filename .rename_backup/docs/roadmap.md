...
This document records the directions we plan to push AutoAS in: a few categories of open
...
AutoAS currently maintains the entire hypothesis tree with a single Coordinator, which can
...
AutoAS currently optimizes around a single benchmark, and the experience it gains is hard
...
AutoAS's optimization objective is currently limited to a scalar score. We are considering
...
and agent evaluation benchmarks, so that AutoAS can support a wider range of optimization
settings.
...
[discussion](https://github.com/RUC-NLPIR/AutoAS/discussions) or see
