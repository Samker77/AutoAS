...
AutoAS's workflow is to take a task that prints a score and iteratively edit the code, run
the eval, and keep the changes that improve the score. The **benchmark zoo** is a collection
of such tasks, each packaged in one standard format so it can be handed directly to AutoAS to
optimize. It lives in [`autoas-zoo/`](https://github.com/RUC-NLPIR/AutoAS/tree/main/autoas-zoo),
one folder per benchmark.

## What it's for

- **Ready-to-run optimization tasks.** Each benchmark ships an eval script and a baseline;
  point AutoAS at it and it begins optimizing.
...
- a **README** — the task description: what the task is, the metric, and what AutoAS may edit;
  read by AutoAS during intake;
- **baseline code** — the starting point for optimization, and the only part AutoAS may edit
  (e.g. `solution.py`);
- an **eval script** — prints one `score:` line when run; it is protected, so AutoAS cannot
  modify it.

AutoAS's loop is: edit the baseline → run the eval → keep the change if the score improved,
and repeat.
...
| Purpose | Command |
| --- | --- |
| List the benchmarks | `autoas benchmark list` |
| Run AutoAS on a benchmark | copy it out of the repo, `git init`, run `autoas` inside it |
| Verify a benchmark's structure | `autoas benchmark verify <dir>` |
| Make your code a runnable benchmark | `autoas benchmark scaffold <dir>` |
| Find & build a benchmark from a request | `autoas benchmark add "<request>"` (or a repo URL) |

Running one:

```bash
cp -r autoas-zoo/algotune_knn /tmp/algotune_knn   # copy out of the AutoAS repo
cd /tmp/algotune_knn && git init -q && git add -A && git commit -qm baseline
autoas                                             # confirm the task, then it iterates
```
