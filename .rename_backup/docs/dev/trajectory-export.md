...
`.autoas/sessions/*/trajectory.jsonl`.
...
Lesson: for RL you need token-level fidelity, not coarse decision logs. AutoAS
...
4. `autoas export <session> traj.jsonl` keeps working for old runs (offline).
