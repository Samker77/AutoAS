# Design note — `autoas benchmark add` (the collection feature)
...
`autoas benchmark add "<spec>"` — point AutoAS at a benchmark you name (a paper, a GitHub
...
*draft* zoo Task Pack that passes `autoas benchmark verify` — then hand it to a human to
...
The collector is its **own top-level agent/pipeline**, parallel to `autoas run`, built on the
...
| Runtime | its own CLI (`autoas benchmark add`), its own process; never shares a research run's idea tree / session |
| Filesystem | works in the global cache + a scratch worktree; only touches `autoas-zoo/` on human accept |
...
first, not the second. The core of AutoAS is the research loop (`autoas run`). Collection is a
**supporting tool** on the external-resources line (方向二) — it *stocks the zoo shelf*; the
zoo is a resource that *feeds* `autoas run`. **It produces an input to AutoAS; it is not
another AutoAS.** Guardrails that keep the hierarchy clear (so the build doesn't drift):

- **CLI**: it stays `autoas benchmark add` — a tool subcommand under the `benchmark` noun,
  sibling to `verify`/`list`. Never a top-level `autoas add` or a headline command. `autoas
...
- **Downloads live in a global cache**: `~/.autoas/cache/benchmarks/<name>/` — reused across
...
3. **Angle decomposition + the freeze axis** — the *same dataset is climbed from different
   ...
   AutoAS's ideation is mechanism-agnostic: give it a broad `edit:` + a budget and its Idea Tree
...
- **Stable core** *(benchmark / synthetic-anchored, e.g. `algotune_knn`)* — AutoAS's own
...
The core keeps AutoAS honestly regression-tested; the frontier tracks the field. Work-first is the
...
roadmap explicitly says AutoAS is **not**. The line that keeps it legitimate:
...
spec ─▶ Stage 0 Survey & Disambiguate (literature survey + search)
        ...
        Stage 1 Acquire (deterministic + download layer)        ── Acquirer.acquire()
          → clone/download into ~/.autoas/cache/benchmarks/<name>/, record provenance
        ...
        Stage 4 Verify + HUMAN accept
          → final `autoas benchmark verify`; show draft pack + report; human edits/accepts
          → on accept: move into autoas-zoo/<name>/   (never auto-accept)
```
...
class Acquired:       # produced by Stage 1
    cache_dir: Path   # ~/.autoas/cache/benchmarks/<name>/
...
1. **Stage 2 generality** — "make a research repo run and emit a score on a clean held-out
   split". Research-hard, but it is *AutoAS's home turf* (the executor already does this) and
...
1. **Shared spine + git modality**: global cache + manifest, `CreatePackTool`, the
   `collect.py` pipeline, `GitRepoAcquirer`, Stage prompts, `autoas benchmark add` CLI →
