...
感谢你有兴趣改进 AutoAS。本页介绍参与代码库和文档工作的基础。
...
!!! note "打包细节"
    ...
    `autoas` 的名字安装和导入。当你新增一个**子包**（一个带 `__init__.py` 的新目录）时，
...
1. 为你的改动创建一个分支。
2. 让改动保持聚焦；与周围代码风格一致。
3. 验证 CLI 仍然可用（`autoas version`、`autoas doctor`）；若你动了文档，确认 `mkdocs build`
   无误。
4. 开一个 pull request，说明动机与改动。

## 引用

如果你在研究中使用了 AutoAS，请引用论文：
