...
Real research starts from what's already known. AutoAS can pull in external
...
```bash
autoas idea-check "your hypothesis in one sentence"
```

See [`autoas idea-check`](cli.md#autoas-idea-check) for options.
...
!!! note "Exa via MCP"
    ...
    `pip install 'autoas-agent[mcp]'`. The plain `exa` backend hits the same
...
Older configs keep working unchanged. When `backends` is empty, AutoAS maps the
...
- [`autoas idea-check`](cli.md#autoas-idea-check) — one-off novelty check.
