...
!!! tip "In a hurry? You can skip this page"
    Most people configure AutoAS exactly once: run `autoas setup`, pick a model, done.
...
This page is written for someone who has never used AutoAS. It answers three questions, in
order:
...
=== "Just trying it out"
    ...
    Run `autoas setup` to choose a model, then `autoas` to start. You do **not** need a
...
=== "Running a real study"
    ...
    Do `autoas setup`, then skim [What you can configure](#what-you-can-configure) and
...
| --- | --- | --- | --- |
| **Essential** | `provider`, `model`, `api_key`, `base_url` | Which LLM AutoAS uses and how to reach it | You must pick a model and supply a key once. |
| **Important** | `max_cycles` | How many experiments before AutoAS stops and writes the report | The main time/cost knob. Higher = longer, deeper search. |
...
| **Advanced** | `plugin`, `plugin_profile` | Retarget AutoAS to a domain (eval rules, protected files, budget bundle) | You run the same kind of benchmark often. See [Plugins](plugins.md). |
...
1. **`autoas setup`** — a one-time wizard that saves your model globally. *Most people only
   ever use this.*
2. **`autoas config`** — view or edit that global file later.
...
```console
$ autoas setup
autoas setup — let's configure your model (one time).
...
Done. Saved to ~/.autoas/config.yaml
```
...
After this, just run `autoas`.

### 2. Inspect or edit the global config: `autoas config`

```bash
autoas config show           # print the effective config (secrets masked)
autoas config path           # where the file lives
autoas config init --provider openai --model gpt-5 --api-key dummy   # write it non-interactively
```

`autoas config init` is the scriptable sibling of the wizard — handy for setting up a local
gateway in one line:

```bash
autoas config init --provider litellm --model qwen-72b \
  --base-url http://localhost:4141 --api-key dummy
```

### Providers

Pick **one** API type. The value you give to `autoas setup` / `--provider` is one of exactly
three:
...
!!! tip "Keep keys out of files"
    ...
    file. `autoas setup` stores your global key under `~/.autoas/` with the rest of the config.

!!! warning "Experimental: ChatGPT subscription login (`openai-oauth`)"
    ChatGPT Plus/Pro/Team subscribers can drive AutoAS with their subscription instead of a
    pay-per-token key. Run `autoas login openai` to sign in through the browser; the token is
    stored in `~/.autoas/oauth/openai.json` and refreshed automatically. This writes:
    ...
    Manage the session with `autoas login status` / `autoas login logout`. Requests go to the
...
!!! warning "Experimental: Claude subscription login (`anthropic-oauth`)"
    Claude Pro/Max subscribers can drive AutoAS with their subscription instead of a
    pay-per-token key. Run `autoas login claude` to sign in through the browser, then paste
    the code the callback page shows; the token is stored in `~/.autoas/oauth/anthropic.json`
...
When a project needs its own durable settings, drop a YAML file in it. AutoAS auto-detects
`research_config.yaml`, `autoas.yaml`, or `autoresearch.yaml` in the target directory (or
...
# ── Orchestration ─────────────────────────────────────
max_cycles: 12                   # experiments before AutoAS finalizes and reports
...
!!! tip "Search backends and grounded ideation"
    AutoAS can use literature and the open web in two independent, default-off
...
```bash
autoas run --max-cycles 20 --mode review --no-webui
```

Common ones: `--max-cycles N`, `--max-turns N`, `--mode MODE`, `--webui-port N`,
`--no-webui`. See the [CLI reference](cli.md#autoas-run) for the full list.
...
| Key | Meaning |
| --- | --- |
| `max_cycles` | Maximum number of completed / skipped / failed idea experiments before AutoAS finalizes and writes the report. Override per-run with `--max-cycles`. |
...
Two top-level keys retarget AutoAS to a domain without touching code:
...
```text
built-in defaults  <  plugin overrides  <  plugin profile  <  global setup (~/.autoas)  <  project config  <  CLI flags
```
...
```bash
autoas config show   # confirm provider/model/key are what you expect (secrets masked)
autoas doctor        # check PATH, Python, git, and that your API key resolves
```

`autoas doctor` is the fastest way to catch a missing key or unreachable gateway before a
run starts.
