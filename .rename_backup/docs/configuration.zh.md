...
!!! tip "时间有限？本页可跳过"
    大多数人只配置 AutoAS 一次：运行 `autoas setup`，选定 model 即可。下面的一切，都是当你想
    **换 model**、**设时间/成本预算**、**加入人工监督**，或**对准某个特定领域**时才需要。需要时
    再回来查阅。

本页是写给从没用过 AutoAS 的人的。它按顺序回答三个问题：
...
=== "只是试试"
    ...
    运行 `autoas setup` 选一个 model，然后 `autoas` 开始。你**不**需要配置文件或任何参数。若有兴趣，可
    阅读[最重要的几项设置](#the-settings-that-matter-most)，其余忽略即可。

=== "做一项真实研究"

    先 `autoas setup`，然后浏览[你能配置什么](#what-you-can-configure)和[设一个预算](#budgets-and-timeouts)。
...
| 层级 | 设置 | 它控制什么 | 你为何会改它 |
| --- | --- | --- | --- |
| **必要** | `provider`、`model`、`api_key`、`base_url` | AutoAS 用哪个 LLM、怎么连上 | 你必须选一个 model 并一次性提供 key。 |
| **重要** | `max_cycles` | AutoAS 停下并写报告前跑多少个实验 | 控制时间与成本的主要参数。值越大，搜索越久、越深入。 |
...
| **高级** | `plugin`、`plugin_profile` | 把 AutoAS 重定向到一个领域（评测规则、保护文件、预算包） | 你经常跑同一类基准。见[插件](plugins.md)。 |
...
1. **`autoas setup`** —— 一次性向导，全局保存你的 model。*大多数人只会用到这个。*
2. **`autoas config`** —— 之后查看或编辑那个全局文件。
3. **项目配置文件** —— 随某个项目一起携带的持久设置。
4. **CLI 参数** —— 单次运行的一次性覆盖。
5. **对话里的斜杠命令** —— 为本次运行挑一个插件或技能，无需文件。

### 1. 你的 model：`autoas setup`

配置好它的最快方式。它问四个问题并写出 `~/.autoas/config.yaml`：
...
### 2. 查看或编辑全局配置：`autoas config`

```bash
autoas config show           # 打印生效配置（密钥已遮蔽）
autoas config path           # 文件位置
autoas config init --provider openai --model gpt-5 --api-key dummy   # 非交互式写入
```

`autoas config init` 是该向导的可脚本化版本——一行命令即可配置好一个本地网关：
...
选**一种** API type。你给 `autoas setup` / `--provider` 的值，恰好是以下三者之一：
...
!!! tip "别把 key 放进文件"
    优先用环境变量（`${ANTHROPIC_API_KEY}`），而非把密钥粘进文件。`autoas setup` 会把你的全局 key
    连同其余配置一起存在 `~/.autoas/` 下。

!!! warning "实验性：用 ChatGPT 订阅登录（`openai-oauth`）"
    ChatGPT Plus/Pro/Team 订阅用户可以用月费订阅来跑 AutoAS，而不必用按量计费的 API key。
    运行 `autoas login openai` 通过浏览器登录；token 存在 `~/.autoas/oauth/openai.json` 并自动刷新。
    它会写入：
    ...
    用 `autoas login status` / `autoas login logout` 管理会话。请求发往 ChatGPT 后端
...
当一个项目需要它自己的持久设置时，往里放一个 YAML 文件。AutoAS 会自动检测目标目录里的
`research_config.yaml`、`autoas.yaml` 或 `autoresearch.yaml`（或用 `--config PATH` 传入）。这里的
...
# ── 编排 ──────────────────────────────────────────────
max_cycles: 12                   # AutoAS 定稿并报告前的实验数
...
!!! tip "检索后端与接地 ideation"
    AutoAS 能在两条互相独立、默认关闭的 lane 中使用文献与开放网络——**接地 ideation**
...
完整列表见 [CLI 参考](cli.md#autoas-run)。
...
| 键 | 含义 |
| --- | --- |
| `max_cycles` | AutoAS 定稿并写报告前，完成/跳过/失败的想法实验的最大数量。按次用 `--max-cycles` 覆盖。 |
...
```text
内置默认  <  插件覆盖  <  插件 profile  <  全局 setup（~/.autoas）  <  项目配置  <  CLI 参数
```
...
```bash
autoas config show   # 确认 provider/model/key 是你预期的（密钥已遮蔽）
autoas doctor        # 检查 PATH、Python、git，以及你的 API key 能否解析
```

`autoas doctor` 是在运行开始前抓出缺失 key 或不可达网关的最快方式。
