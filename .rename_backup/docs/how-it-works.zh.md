...
AutoAS 把一个长周期目标转化为结构化、累积式的搜索。本页解释核心思想：两个智能体、AutoAS 循环、
...
想法树是 AutoAS 的持久记忆。每个节点是一个假设，附带其状态、它产生的证据以及从中提炼的洞见：
...
只收到一份简洁、相关的摘要，而不是把先前的每条消息全部传给它。正是这一点，让 AutoAS 能在一项长期
研究中持续推进，而不被自身历史淹没。
...
默认情况下 AutoAS 拒绝从非基础分支启动；当你有意要在某个特性分支上推进时，用
...
AutoAS 默认完全自主运行，但你可以通过交互模式引入监督：
