...
!!! tip "首次运行的捷径"
    如果你在配置任何东西之前就启动了一次运行，AutoAS 会在交互式终端中检测到缺失的配置，并自动
    引导你走完 `autoas setup`。
...
使用 AutoAS 的方式，就是在你的项目目录里运行 `autoas`：
...
!!! tip "还没有项目？试试自带的示例"
    ...
    任务——在匹配参考输出的前提下，让一个暴力 k-NN 求解器跑得更快。请在你的 AutoAS
...
运行完成后，AutoAS 写出一个 `REPORT.md`，并打开一个可选的只读问答提示，让你就这次已完成的
研究进行追问（用 `--no-followup` 关闭）。所有产物——想法树、检查点、日志以及每个实验的分支——都位于
`<project>/.autoas/sessions/<run_name>/` 下。
