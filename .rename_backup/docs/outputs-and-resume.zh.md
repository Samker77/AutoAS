...
AutoAS 记录一次运行产生的一切，让你能检视它、复现它，并从你停下的地方继续。
...
!!! tip "复用一套设置"
    ...
    并命名为 `autoas.yaml`——AutoAS 下次会自动加载它。见
    [准备基准 → 保存并复用一套设置](preparing-a-benchmark.md#saving-and-reusing-a-setup)。

## `REPORT.md`

一次运行结束时，AutoAS 写出一个 `REPORT.md`——这项研究的人类可读成稿：尝试了什么、什么奏效、
每个结论背后的证据，以及最终结果。用 `autoas report` 处理它，运行过程中用 `/report` 斜杠命令
打印产物路径。

默认情况下 AutoAS 随后会打开一个**只读问答提示**，让你就这次已完成的运行进行追问（用 `--no-followup`
关闭）。
...
`autoas` 启动后、真正发起运行*之前*的那段规划对话，本身也会每轮自动保存，方便你离开后接着聊。
每段对话有自己的目录，位于启动目录下：
...
- **`autoas --continue`**（`-C`）—— 重新载入最近一段*未结束*的对话并接着聊，类似 `claude -c`。
