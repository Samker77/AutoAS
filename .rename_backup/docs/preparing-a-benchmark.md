...
You don't write a config to point AutoAS at a task. There is really only **one hard
requirement**: a repo where AutoAS can **run something to get a score**. Everything else —
what the metric means, what's off-limits, how ambitious to be — you settle in a short
**intake chat** when you launch `autoas`. No hand-written YAML, no eval contract to author,
and **no README required**.

!!! tip "Prefer to learn by example?"
    [`examples/algotune_knn`](https://github.com/RUC-NLPIR/AutoAS/tree/main/examples/algotune_knn)
...
└── train.py         # your starting-point code (AutoAS edits this)
```

The one thing AutoAS needs is a command that **prints a metric on a line it can read**. Your
...
A simple working baseline already in the repo is ideal — it gives AutoAS a number to beat
...
!!! tip "Only have code? Intake can plan the eval setup with you"
    ...
    repo is just code, launch `autoas` anyway: in the intake chat the agent asks what
...
!!! tip "Let AutoAS do the plumbing after approval"
...
AutoAS needs a plain-language picture of the task. You can supply it **either way**:

- **In the intake chat** — the simplest path. Launch `autoas`, and the agent reads your
...
- **What's off-limits** — anything AutoAS must not modify, such as `data/` or the eval
  script itself.
...
That's the whole setup. Everything AutoAS needs — how to evaluate, what to protect, what
counts as "better" — comes from your repo plus the short confirmation, not from a config
file you maintain.

!!! note "Held-out discipline"
    AutoAS iterates on a **dev** signal but only keeps a change if it improves a **held-out**
...
- **Continue the same study** — each run is checkpointed under
  `<project>/.autoas/sessions/<run_name>/`. Just launch `autoas` again in the project and
  pick the past run from the resume list (or `autoas --resume --run-name <name>`). This
...
- **Repeat with the same settings, fresh** — every run writes its resolved config to
  ...
  Copy it to your project root as `autoas.yaml` (or `research_config.yaml`) and AutoAS
...
- **Set defaults once for every project** — run `autoas setup` to write
  `~/.autoas/config.yaml`; those become your global CLI defaults.
- **Re-run the exact same goal unattended** — pass the instruction headlessly and skip the
  chat entirely:

    ```bash
    autoas run "maximize dev score from bash eval.sh; don't touch data/ or eval.sh" \
      --yes --yes-cwd ./my_task
    ```
...
[plugin](plugins.md). AutoAS ships one for Kaggle / MLE-bench (`mle_kaggle`) as a worked
example.
