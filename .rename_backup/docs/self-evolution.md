...
AutoAS remembers what it learns. Every run leaves **concrete, situational findings**
...
At the end of the run, AutoAS consolidates these (plus, optionally, findings it mines
from the run itself) into an **`EXPERIENCE.md`** inside that run's session folder:

```
.autoas/sessions/<run_name>/
...
AutoAS: I found experience from a past run on this project
...
rules. AutoAS's normal dev/test discipline still filters them — a stale or wrong prior
...
exploration. AutoAS does **not** try to abstract findings into generic principles
