# Arbor V2 · MLE-Bench Lite 任务运行指南

> 给跑任务的组员：本仓库是 **V2 快照**。你们已经跑过原始 arbor，这里只讲 **V2 改了哪里**、**如何快速起一个新 MLE-Bench Lite 任务**、**有哪些坑**。
...
git clone https://github.com/Samker77/Arbor-Scientist.git
cd Arbor-Scientist
python -m venv .venv && source .venv/bin/activate   # 或复用你们已有的 arbor venv
pip install -e .
arbor doctor        # 验证 PATH / git / API keys 就绪
```

> 如果你之前已经装过原版 arbor：**先 `pip install -e .` 重装到本快照**，再 `arbor doctor` 确认。
...
参考模板：`arbor-zoo/_template/`（有 `eval.sh` / `eval.py` / `README.md` / `task.py` 的最小骨架）。
...
arbor --yes "maximize the score from eval.sh without touching data/ or eval.sh" \
      --yes-cwd . \
      --run-name v2-<task名> \
      --no-dashboard-input
```
...
- **浏览器实时监控**：`arbor web <run_name>`（或启动时自动打开，端口约 8765）
- **终端状态**：运行中 `/status` `/tree` `/evidence` `/branches` `/cost` `/report`
- **中断恢复**：`arbor --resume --run-name <run_name>`（从想法树断点继续）
...
```
<task_dir>/.arbor/sessions/<run_name>/
...
| 现象 | 原因 / 处理 |
|---|---|
| `arbor doctor` 报 API key 缺失 | `arbor setup` 配好 Qwen 端点 + key，或写进任务 `research_config.yaml` |
