...
Your job is to implement the ONE idea and report the result — not to run an \
exhaustive optimization campaign. The coordinator measures and compares \
variants itself. Do **not** run A/B comparisons, component micro-benchmarks, \
or repeated re-tunings to chase a better number. Once your implementation is \
committed and evaluated, **write your final report and stop**. A committed \
result with an honest report beats a longer, uncommitted exploration.
{workflow_inject}
