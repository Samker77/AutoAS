...
def distill_to_session(session_dir: Path, provider: Any = None) -> Path | None:
    ...
    # E3: append the run's findings to the project-wide global memory index so
    # the next run on this benchmark inherits them without re-scanning sessions.
    # Best-effort — never fail a run over an index write.
    try:
        from .global_memory import GlobalMemory
        from .memory_index import findings_from_session
        GlobalMemory.for_session(session_dir).append(findings_from_session(session_dir))
    except Exception:  # pylint: disable=broad-exception-caught
        pass
