...
class SearchConfig(BaseModel):
    ...
    # E1 dedup guard: when a new idea is added, flag hypotheses similar to
    # already-known nodes (pruned/done/merged/failed) *outside the idea's own
    # lineage*. Advisory only — the idea is still added; the coordinator sees
    # the evidence and can deliberately re-test (and say why). Deterministic.
    dedup_warning_on_add: bool = True
    # Cosine-similarity threshold above which a new idea is flagged. Calibrated
    # on the counterfactual replay: 0.4 catches re-treads without flagging
    # distinct directions that merely share a common word.
...
class CoordinatorConfig(ProxyModel):
    ...
    # Executor convergence — stop executors drifting into side-experiments after
    # they have committed a working implementation (see AgentConfig). Each of
    # these maps 1:1 onto an AgentConfig field for the spawned executor.
    executor_converge_after_commit: bool = True
    executor_converge_idle_turns: int = 5
    executor_converge_horizon: int = 4
    executor_converge_max_nudges: int = 2
    ...
    def to_executor_config(self, idea_id: str, idea_hypothesis: str) -> "AgentConfig":
        ...
        # Executor convergence: only executors opt in (the coordinator must
        # never converge early).
        executor_config.converge_after_commit = self.executor_converge_after_commit
        executor_config.converge_idle_turns = self.executor_converge_idle_turns
        executor_config.converge_horizon = self.executor_converge_horizon
        executor_config.converge_max_nudges = self.executor_converge_max_nudges
