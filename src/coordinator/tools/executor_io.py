...
def _build_executor_prompt(
    *,
    ...
    parts.append(
        "## Budget & Convergence\n\n"
        f"You have a hard cap of **{max_turns} turns**. Your deliverable is a "
        "**committed implementation** plus a **final report** — not an exhaustive "
        "search.\n\n"
        "- **Do NOT run A/B comparisons of multiple implementation variants, "
        "component benchmarks, or profile wars.** Variant measurement is the "
        "coordinator's job; it re-measures your committed branch independently. "
        "You implement the ONE idea faithfully and report the result.\n"
        "- As soon as your implementation is committed and you have run the "
        "evaluation, **write your final report**. Do not start new experiments "
        "to squeeze out a better number.\n"
        "- **Reserve the last turns for the report.** If you are near the turn "
        "cap with a committed, evaluated implementation, finalize immediately — "
        "the coordinator will measure the committed branch itself."
    )
