...
class TreeAddNodeTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> str:
        ...
        # E1 soft warning: flag hypotheses that re-tread a known direction.
        # Advisory — the idea is still added; the warning rides on the response.
        warning = ""
        if self._config is not None and self._config.search.dedup_warning_on_add:
            try:
                from ..dedup_guard import dedup_warning
                warning = dedup_warning(
                    self._tree, parent_id, hypothesis,
                    threshold=self._config.search.dedup_threshold)
            except Exception as exc:  # noqa: BLE001
                log.warning("E1 dedup guard failed: %s", exc)
