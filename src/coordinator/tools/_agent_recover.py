...
def filter_sources_to_visited(
    ...
    """Drop sources whose URL was never visited.

    Returns ``(kept, dropped_indices)``:

    - ``kept`` is a list of ``(index, source)`` pairs, where ``index`` is the
      source's original 1-based position in ``sources`` — the ``[n]`` numbering
      the research agent used in its ``details``. Rendering keeps this numbering,
      so body citations stay aligned even after unvisited entries are dropped.
    - ``dropped_indices`` lists the 1-based positions that were dropped, so the
      reader can see exactly which ``[n]`` markers no longer resolve.
    ...
    """
    if not visited:
        return [(i, s) for i, s in enumerate(sources, 1)], []
