...
def _render_digest(parsed: dict[str, Any], dropped: list[int] | None = None) -> str:
    """Render the research assistant's JSON into a digest the coordinator reads.

    Sources keep their ORIGINAL ``[n]`` numbering — the indices the agent used
    in its ``details`` — so the coordinator's ``TreeAddNode(grounding=...)``
    citations stay aligned with what the findings actually reference.
    ``dropped`` lists the original indices that were discarded because they
    were never actually visited; the note names them so a dangling ``[n]`` in
    the findings is explainable instead of silently pointing at the wrong
    source.
    """
    ...
    if dropped:
        ...
        if len(dropped) == 1:
            lines.append(
                f"_Note: 1 source ({shown}) was dropped because it was never "
                f"opened during the search._"
            )
        else:
            lines.append(
                f"_Note: {len(dropped)} sources ({shown}) were dropped because "
                f"they were never opened during the search._"
            )
...
async def _run_research(
    *,
    ...
    # Integrity: drop sources that were never actually opened, keeping their
    # original [n] numbering so body citations stay aligned with the list.
