...
def write_messages(
    ...
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    # ``_est_*`` keys are the A4 token-estimate cache (context.py): process-local
    # and reconstructed on demand. Persisting them would duplicate each message's
    # content (the cache holds a ref to it) and stale-refs on resume.
    payload = "".join(
        json.dumps(
            {k: v for k, v in m.items() if not k.startswith("_est_")},
            ensure_ascii=False,
        ) + "\n"
        for m in messages
    )
