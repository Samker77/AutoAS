...
from ..core.context import _invalidate_message_tokens
...
def _rewrite_assistant_message(msg: dict[str, Any]) -> bool:
    ...
    if modified:
        ...
        _invalidate_message_tokens(msg)
    return modified
...
def _rewrite_skill_tool_results(
    ...
    for blk in blocks:
        ...
        stubbed += 1
    if stubbed:
        # Content mutated in place (same list object) — identity check can't
        # see it, so invalidate the A4 estimate cache explicitly.
        _invalidate_message_tokens(msg)
    return stubbed
