...
def _extract_metric_value(m: re.Match) -> str:
    """Value from a metric regex match, tolerating patterns without a capture group.

    Patterns are documented as "one capture group for the value", but an agent
    may supply a bare pattern (no group) — fall back to the whole match rather
    than raising ``IndexError: no such group`` and killing the tool run.
    """
    try:
        return m.group(1)
    except IndexError:
...
def _extract_line_metrics(line: str, patterns: list[tuple[str, re.Pattern]]) -> str:
    ...
    for metric_name, pattern in patterns:
        m = pattern.search(line)
        if m:
            found.append(f"{metric_name}={_extract_metric_value(m)}")
    return ", ".join(found)
