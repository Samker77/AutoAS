...
# Per-message estimate cache (A4). Framework metadata keys on messages are
# stripped before any provider sees them (claude._cache_messages drops ``_*``;
# openai_compat._convert_messages rebuilds fresh dicts), and ``_est_`` keys are
# excluded from checkpoints (coordinator.checkpoint.write_messages), so these
# keys are safe to store directly on the message dict.
_EST_TOKENS_KEY = "_est_tokens"          # cached count_tokens sum for one message
_EST_TOKENS_REF_KEY = "_est_tokens_ref"  # strong ref to the content this estimate covers


def _estimate_single_message_tokens(msg: dict[str, Any], provider: LLMProvider) -> int:
    """Rough token estimate for one message (uncached)."""
    total = 0
...
def _estimate_message_tokens(messages: list[dict[str, Any]], provider: LLMProvider) -> int:
    ...
    A4 optimization: per-message estimates are cached under ``_est_tokens`` so
    the (expensive) provider tokenizer runs once per content snapshot instead
    of once per call. ``maybe_compact`` recomputes ``_current_tokens`` up to 5x
    per invocation and once before every LLM call; with many messages that
    repeated O(N) recounting used to dominate the per-call cost. Now each call
    only tokenizes the delta (new/changed messages); unchanged messages are
    validated by a single O(1) identity check.

    The estimate is keyed to the exact content object it was computed from
    (``_est_tokens_ref is content``). Every content *replacement* — compaction
    layer rebuilds, ``orchestrator._append_resume_nudge``, ``context_prune``
    list rewrites — allocates a new object, so the cached value is skipped and
    recomputed. Holding the ref (not just ``id()``) keeps the old content alive
    so its address can never be reused by a different object, making the ``is``
    check exact. The single in-place *block* mutation (``context_prune`` stubs
    a tool result inside an unchanged list) cannot be seen by identity, so that
    site calls :func:`_invalidate_message_tokens` explicitly.
    """
    total = 0
    for msg in messages:
        cached = msg.get(_EST_TOKENS_KEY)
        if cached is not None and msg.get(_EST_TOKENS_REF_KEY) is msg.get("content", ""):
            total += cached
            continue
        est = _estimate_single_message_tokens(msg, provider)
        msg[_EST_TOKENS_KEY] = est
        msg[_EST_TOKENS_REF_KEY] = msg.get("content", "")
        total += est
    return total


def _invalidate_message_tokens(msg: dict[str, Any]) -> None:
    """Invalidate the A4 token-estimate cache after an in-place content mutation.

    Needed by code that mutates a message's content *in place* — e.g.
    ``context_prune._rewrite_skill_tool_results`` stubs a ``tool_result`` block
    inside an unchanged list, which the identity check above cannot detect.
    """
    msg.pop(_EST_TOKENS_KEY, None)
    msg.pop(_EST_TOKENS_REF_KEY, None)
