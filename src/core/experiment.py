...
async def _run_cmd(cmd: str, cwd: str, timeout: int = 30) -> tuple[str, int]:
    ...
    Windows-safe: commands built with shlex.quote() must not go through
    cmd.exe (it does not strip POSIX single quotes, so quoted paths would
    reach git literally). Parse argv with shlex.split and exec directly;
    fall back to the shell only if that fails.
    """
    proc = None
    try:
