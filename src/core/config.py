...
class AgentConfig(ProxyModel):
    ...
    # ── Executor convergence (post-commit finalize enforcement) ──────
    # Executors drift: after committing a working implementation they keep
    # launching A/B side-experiments until they exhaust max_turns. When enabled,
    # the agent loop nudges the executor to write its final report once it has
    # (a) committed real code and then gone ``converge_idle_turns`` turns without
    # another code commit, or (b) reached ``max_turns - converge_horizon``.
    # After ``converge_max_nudges`` ignored nudges the loop stops early
    # (stop_reason="converged") and the coordinator's recovery measures the
    # committed branch. Off by default — the coordinator itself must never
    # converge early; only executors opt in via CoordinatorConfig.
    converge_after_commit: bool = False
    converge_idle_turns: int = 5
    converge_horizon: int = 4
    converge_max_nudges: int = 2
