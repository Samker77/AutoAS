...
def record_finding(session_dir: str | Path | None, *, kind: str, about: str, note: str,
                   source: str = "agent", domain: str = "", tags: Iterable[str] = ()) -> None:
    ...
    ``domain`` and ``tags`` are optional metadata (see memory_index.Finding) — they
    stay off the JSON record when empty so existing files stay backward compatible.
    """
