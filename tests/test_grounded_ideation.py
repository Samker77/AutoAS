...
def test_visited_urls_normalizes_versions():
    ...
    ) == ([(1, {"url": "https://www.alphaxiv.org/abs/2203.11171"})], [])
...
def test_filter_drops_unvisited_source():
    ...
    # original 1-based positions preserved: kept keeps its number, dropped is named
    assert dropped == [2]
    assert kept == [(1, {"url": "https://www.alphaxiv.org/abs/2203.11171"})]
...
def test_filter_noop_when_nothing_visited():
    ...
    assert dropped == [] and kept == [(1, {"url": "https://a.com"}),
                                      (2, {"url": "https://b.com"})]
...
def test_research_digest_keeps_original_source_numbering(monkeypatch):
    """Citation-desync regression: after an unvisited source is dropped, the kept
    source must keep its ORIGINAL ``[n]`` number (the index the agent cited in
    ``details``), and the note must name the dropped index — not silently
    renumber, which would make ``details`` point at the wrong source."""
    canned = json.dumps({
        "summary": "s",
        "details": "The concrete approach is [2].",
        "sources": [
            {"title": "Unvisited", "url": "https://www.alphaxiv.org/abs/2604.99999", "note": "n"},
            {"title": "Visited", "url": "https://www.alphaxiv.org/abs/2203.11171", "note": "n"},
        ],
    })
    ...
    tool = ResearchSearchTool(cwd=".", config=_cfg(grounded=True), provider=_Provider())
    digest = asyncio.run(tool.execute(query="x", intent="related_work"))
    # details says "[2]" → the kept source must still be listed as [2], not [1]
    assert "[2] [Visited]" in digest
    assert "[1] [Visited]" not in digest
    assert "([1])" in digest  # note names the dropped index
    assert "Unvisited" not in digest
